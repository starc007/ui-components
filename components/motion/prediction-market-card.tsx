	...
	function choice(
		...
		return (
			<motion.button
				...
				onClick={() => onOutcomeClick?.({ outcomeId: outcome.id, side })}
				whileTap={reduce ? undefined : { scale: 0.96 }}
				transition={SPRING_PRESS}
				className="flex min-h-10 min-w-16 shrink-0 items-center justify-center rounded-xl border border-border bg-background px-3 text-sm font-semibold text-foreground shadow-[0_3px_0_var(--color-border)] transition-colors hover:bg-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
			>
				<NumberTicker
