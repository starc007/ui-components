...
| slug | file | what it does |
|---|---|---|
| `message-bubble` | `components/agents/message-bubble.tsx` | Focused conversational surface (`MessageBubble`) with six visual treatments, independent alignment, spring-scale entry, compact grouping, polymorphic links/buttons, and `MessageBubbleCollapsible` for long responses; assistant examples compose `StreamingResponse` instead of duplicating response behavior |
| `message` | `components/agents/message.tsx` | Composable conversation message (`Message`) with bubble, avatar, header, footer, group, marker, and typing primitives plus a separate `MessageScroller` that follows a nested `StreamingResponse` through resize observation without pulling readers away from older messages |
