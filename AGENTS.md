...
- Resolve which row of a list is active — highlighted, or the owner of a roving `tabIndex` — **during render**, never in a passive effect. An effect lands after the browser paints, so the list is on screen for a frame with the wrong row active, or no row at all: a key pressed in that frame acts on the wrong row, and a roving `tabIndex` chosen in an effect leaves the list untabbable in its first commit and in server-rendered markup. Hold the cursor as the row's **identity**, not its position — a position cannot tell a list that shrank from one that swapped its rows for a different set of the same length — and clear a stale cursor rather than ignoring it, so rows that come back cannot revive it. Step through a functional update so two keys in one batch move two rows. Return the mover and the setter with an identity stable for the component's life — the exhaustive-deps rule will put them in a caller's effect dependencies, and one rebuilt per keystroke re-runs that effect per keystroke. Reset on open during the render that opens (`lib/hooks/use-on-open.ts`), setting only the component's own state: a consumer's callback, a DOM write or a focus move is a side effect and belongs in an effect keyed to `open`. `lib/hooks/use-row-cursor.ts` does all of this and is the ordinary case; `components/motion/combobox/use-active-option.ts` is the same model with wrap-around stepping, `first`/`last`, a disabled-row filter and a not-yet-opened state.
