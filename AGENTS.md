...
| slug | file | what it does |
|---|---|---|
| `composition-chart` | `components/charts/composition-chart.tsx` + `composition-chart/` | Composable normalized stacked bar/area chart with Plot, compact stable-order Legend and useCompositionChart; bar and area views share one preview and usage API; consumer-supplied values/colors, controlled/uncontrolled period inspection, hover and an invisible native keyboard/touch slider over the plot, series pinning, missing-period gaps and reduced-motion-safe bar transitions |
