...
| slug | file | what it does |
|---|---|---|
| `bump-chart` | `components/charts/bump-chart.tsx` + `bump-chart/` | Composable ranking chart with Plot, Legend, and useBumpChart; curved paths, numbered rank dots, gap-aware data, controlled/uncontrolled series pinning, hover/focus isolation, accessible exact-rank table, and reduced-motion-safe reveal |
