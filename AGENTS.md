...
- Resolve which row of a list is active — highlighted, or the owner of a roving `tabIndex` — **during render**, never in a passive effect. An effect lands after the browser paints, so the list is on screen for a frame with the wrong row active, or no row at all: a key pressed in that frame acts on the wrong row, and a roving `tabIndex` chosen in an effect leaves the list untabbable in its first commit and in server-rendered markup. `lib/hooks/use-row-cursor.ts` is the ordinary case; `components/motion/combobox/use-active-option.ts` is the same model with wrap-around stepping, `first`/`last`, a disabled-row filter and a not-yet-opened state.
  ...
  - Clear a stale cursor rather than ignoring it, so rows that come back cannot revive it, and step through a functional update so two keys in one batch move two rows.
  - Return the mover and the setter with an identity stable for the component's life. The exhaustive-deps rule will put them in a caller's effect dependencies, and one rebuilt per keystroke re-runs that effect per keystroke. Keep the values they read in a ref written from a layout effect, never during render.
  - Start a fresh session in the render that opens (`lib/hooks/use-on-open.ts`), setting only the component's own state: a consumer's callback, a DOM write or a focus move is a side effect and belongs in an effect keyed to `open`. A component that instead keeps the query it was open with through the exit, as the combobox does, resets on the next open by that route and needs no transition.
