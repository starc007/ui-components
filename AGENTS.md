...
| `message-scroller` | `components/agents/message-scroller.tsx` | Reader-aware conversation viewport (`MessageScroller`) that follows streamed growth only at the live edge, releases on wheel, touch, keyboard, or manual scrolling, and exposes separate viewport/content props with an accessible region and live transcript |
