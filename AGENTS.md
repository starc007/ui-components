...
- Overlays unmount while closed — `AnimatePresence` around the chrome (transparent full-viewport fixed layers are costly on iOS Safari — see `tests/fixed-overlay-edge-sampling.test.tsx`). One that genuinely must stay mounted hides on the close path only, flipping a hidden state from the exit animation's own completion so no duration has to be kept in sync. Opening must be instant, or focus and measurement effects run against a still-hidden element; re-test open-path behaviour after touching show/hide plumbing.
- An overlay under `AnimatePresence` is still in the document for the length of its exit, so interaction is gated on presence, never on `open` — `open` has already flipped while the chrome is still the topmost thing on the page. `PresenceGate` (`lib/presence-gate.tsx`) reads `useIsPresent()` inside the subtree and hands down a `gate` to spread onto every layer that takes pointer events: it releases pointer events and, through `inert`, focus order and the accessibility tree in the same commit that starts the exit, while the visual exit plays on. A wrapper that never takes pointer events carries `inert={!isPresent}` alone. An overlay that stays mounted instead gates on its own open state, which flips on the same render.
- A `position: fixed` overlay layer either paints a colour, or has no children, or stays off the viewport edges — the same test sweeps every overlay in the library in each state it can be in, so a new one needs a row there. Grouping wrappers that only carry `inert`, a z-index or a presence key take no box at all (`fixed left-0 top-0 size-0`) and let their `fixed` children position themselves; a wrapper that lays the panel out is inset off the edges with the content box its padding used to give it.
