...
| `streaming-response` | `components/agents/streaming-response.tsx` | Renderer-agnostic streamed answer surface (`StreamingResponse`) with a live cursor and completion-only copy, retry, and feedback actions |
| `tool-approval` | `components/agents/tool-approval.tsx` | Human-in-the-loop permission card (`ToolApproval`) with controlled execution states, expandable parameters, allow-once/always-allow/deny actions, and reduced-motion behavior |
| `sources` | `components/agents/sources.tsx` | Collapsible progressive source list (`Sources`) paired with inline `SourceCitation` links; supports custom marks, descriptions, controlled disclosure, and reduced motion |
