...
| `knockout-bracket` | `components/motion/knockout-bracket.tsx` | Page is "Fixtures", a generic tournament-fixtures page whose versions live in the entry's `examples` (404 / Not Found has the same shape). Version one is Knockout Bracket (`KnockoutBracket`): pages one round at a time, later rounds center on their feeders, optional `thirdPlace` match under a rule. Add a fixture style as another `examples` item with its own `installSlug`, and keep `knockout-bracket` as one of them so `/r/knockout-bracket.json` still resolves. Version two is planned: Bracket Wheel (`BracketWheel`, `components/motion/bracket-wheel.tsx`, example slug and `installSlug` `bracket-wheel`, `previewKey` `blocks/bracket-wheel`) draws the same tournament radially, champion at the hub, one ring per round, arc and spoke links, eliminated nodes dimmed |
