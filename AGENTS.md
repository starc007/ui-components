...
| `tooltip` | `components/motion/tooltip.tsx` | Hover/focus tooltip with subtle spring entry, smoothly retargeted placement; content moves with its surface and chart readouts keep a stable width |
...
| slug | file | what it does |
|---|---|---|
| `composition-chart` | `components/charts/composition-chart.tsx` + `composition-chart/` | Composable normalized stacked bar/area chart with Plot, compact stable-order Legend and useCompositionChart; bar and area views share one preview and usage API; consumer-supplied values/colors, controlled/uncontrolled period inspection, hover and an invisible native keyboard/touch slider over the plot, series pinning, shared tooltips with exact values and shares, missing-period gaps and reduced-motion-safe bar transitions |
...
| `order-book` | `components/charts/order-book.tsx` + `order-book/` | Composable snapshot-driven depth ladder with Header, Side, Spread, Balance, and useOrderBook; normalizes price levels, accumulates from best quotes, animates depth updates with stationary numbers, supports custom formatting and reduced motion; demo data stays in the preview |
