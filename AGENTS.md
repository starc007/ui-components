...
| `range-slider` | `components/motion/range-slider.tsx`, `range-slider-fluid.tsx`, `range-slider-wave.tsx`, `range-slider-bubble.tsx`, `range-slider-ruler.tsx` | Five slider designs, one per `examples` entry with its own `installSlug`. `RangeSlider` (base) has tick dots and a bouncy vertical-bar thumb. `FluidSlider` is thumbless — gliding fill with a rounded liquid cap and label that inverts under the fill. `WaveSlider` is equalizer bars cresting around the handle. `BubbleSlider` pops a value bubble that leans/squashes with drag velocity. `RulerSlider` scrolls a scale under a fixed needle with drag momentum. All share value/step/drag/keyboard plumbing from `lib/hooks/use-slider.ts`; controlled/uncontrolled, reduced-motion safe |
