...
| `tooltip` | `components/motion/tooltip.tsx` | Hover/focus tooltip with Floating UI collision-aware positioning, direct frame-coalesced cursor tracking via `followCursor`, opacity-only entry/exit and hoverable static content; position and readable content never animate independently |
