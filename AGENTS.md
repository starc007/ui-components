...
- Resolve which row is active **during render**, never in a passive effect. A passive effect lands after the browser paints, so the list is on screen for a frame with the highlight absent or on the wrong row, and a key pressed in that frame acts on that row. Hold the cursor as the row's **identity**, not its position — a position cannot tell a list that shrank from one that swapped its rows for a different set of the same length — and clear a stale cursor rather than ignoring it, so results that come back cannot revive it. Step through a functional update so two keys in one batch move two rows. See `components/motion/combobox/use-active-option.ts`.
