...
- Overlays that hide while closed (transparent full-viewport fixed layers are costly on iOS Safari — see `tests/fixed-overlay-edge-sampling.test.tsx`) hide on the close path only: either transition `visibility` with a delay, or flip a hidden state from the exit animation's own completion (preferred — no duration to keep in sync). Opening must be instant, or focus and measurement effects run against a still-hidden element; re-test open-path behaviour after touching show/hide plumbing.
